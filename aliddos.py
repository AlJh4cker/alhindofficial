...
def get_Typer():
	...
	global fake_ip
	global hides
	optp = OptionParser(add_help_option=False,epilog=" AL HIND OFFICIAL DDOS ATTACK")
...
def bot_hammering(url):
	try:
		while True:
			...
			print("\033[ AL HIND is attacking...\033[0m")
			time.sleep(.1)
	except:
		time.sleep(.1)
...
#DDOS 1 + MASSEGS
def attack(item):
             with console.status("[bold dark_orange] AL HIND OFFICIAL is Attacking CTRL+Z .....") as status:
                 while True:
                     try:
...
def usage():
    console = Console()
    MARKDOWN=("""
         	     TEAM AL HIND OFFICIAL DDOS ATTACK            """)
...
def usage1():
	...
	usage : python  aliddos.py [-i] [-p] [-l]
            ...
            -d : Hide message
            	Best lvl =3
	example :
	    python  aliddos.py -i 12.345.678 -p 80 -l 3"""
    md = Markdown(MARKDOWN)
    console.print(md,style="bold green")
    #sys.exit()
...
def usage2():
    ...
    MARKDOWN2="""
                	...
                	TEAM AL HIND OFFICIAL ALWAYS ON TOP
    """
...
def usage3():
    ...
    MARKDOWN="""
    Programmer : SIR ALI
    Made for : TEAM AL HIND OFFICIAL 
    Instagram : @S1R_4L1 
    Tool Information : TEAM AL HIND OFFICIAL TOOL
"""
    md = Markdown(MARKDOWN)
    console.print(md,style="bold green")
